...
PGM.N=1200; % grating line density in lines/mm
h=7.5; % groove height in nm
alpha=10; % trapezoidal angle in degrees measured relative to surface
gamma_D=0.74; % groove width / groove d-spacing (groove measured on top)
num_slices=4; % number of slices defined to approximate %
...
d=40; % multilayer d-spacing in nm (or thickness of the single-layer)
...
for i=1:numE
    ...
    PGM.energy=energy(i);
    % define grating
    [grating,def_time(i)] = MLgrating_def_lam(PGM.N,h,alpha,gamma_D,num_slices,d,gamma_d,num_periods,materials,datadir,PGM.wavelength);
