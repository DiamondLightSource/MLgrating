function [grating,time_elapsed] = MLgrating_def_lam(N,h,alpha,gamma_D, ...
...
% calculate tangent of trapezoidal angle once to avoid rounding
% errors
...
for j=1:num_strata
    ...
    % grating bottom
    if heights(j) >= ML_thickness
        stripe.pmt_index = 1; % Vacuum's permittivity index
    elseif n == length(h_bottoms)
        stripe.pmt_index = 1; % Vacuum's permittivity index
    elseif heights(j) < h_bottoms(n+1) 
        if mod(n,2)
        stripe.pmt_index = 3; % 1st layer's permittivity index
        else
        stripe.pmt_index = 4; % 2nd layer's permittivity index
